...
import re
import codecs
from typing import IO, Any, Callable, Dict, Optional, TypeVar, Union
import typing
import warnings
...
class SinkParser:
    def __init__(
        ...
        if genPrefix:
            ...
            store.setGenPrefix(genPrefix)  # type: ignore[attr-defined] # pass it onq
    ...
    def qname(self, argstr: str, i: int, res):
        ...
        if i < len_argstr and argstr[i] == ":":
                ...
                if c == "\\" and not lastslash:  # Very rare.
                    ...
                    if lastslash:
                        if c not in escapeChars:
                            raise BadSyntax(
                                self._thisDoc,
                                self.line,  # type: ignore[attr-defined] # TODO FIXME
                                argstr,
                                i,
                                "illegal escape " + c,
                            )
            ...
            if lastslash:
                raise BadSyntax(
                    self._thisDoc, self.line, argstr, i, "qname cannot end with \\" # type: ignore[attr-defined] # TODO FIXME
                )
...
class TurtleParser(Parser):
    ...
    def parse(
        ...
        # print(f"source={source!r}")
        # print(f"source.getCharacterStream()={source.getCharacterStream()!r}")
